...
func (p *Parser) resolveTSImport(name, fromRelPath string) (relPath, origin string, ok bool) {
	...
	rel, ok := p.resolveTSModulePath(b.spec, fromRelPath)
	if !ok {
		return "", "", false
	}
	return rel, b.origin, true
}

// resolveTSModulePath resolves a relative module specifier (e.g. "./util"), as
// written in fromRelPath, to an in-tree file: a same-named source file first,
// then a directory index. Factored out of resolveTSImport so calls.go's
