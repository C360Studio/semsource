...
// imageSourceSchema defines the configuration schema for the image-source component.
var imageSourceSchema = component.GenerateConfigSchema(reflect.TypeOf(Config{}))

// sourceCfg is a minimal handler.SourceConfig adapter for the image handler.
...
func (s *sourceCfg) GetType() string             { return "image" }
...
// Component implements the image-source processor.
// It delegates all filesystem operations to the existing handler/image package,
...
// NewComponent creates a new image-source processor component.
func NewComponent(rawConfig json.RawMessage, deps component.Dependencies) (component.Discoverable, error) {
	...
	opts := []imghandler.Option{imghandler.WithOrg(config.Org)}
	...
	if config.FileStoreRoot != "" {
		...
		fs = s
		opts = append(opts, imghandler.WithStore(s))
	}

	h := imghandler.New(opts...)
...
// Start performs the initial image ingest across all configured paths, then
...
// ingestOnce runs a single ingest pass: calls IngestEntityStates on the image
...
func (c *Component) ingestOnce(ctx context.Context) error {
	...
	if err != nil {
		...
		return fmt.Errorf("image handler ingest: %w", err)
	}
...
// paths and re-publishes changed image entities. Returns the cancel func for
...
func (c *Component) startWatching(ctx context.Context) context.CancelFunc {
	...
	if err != nil {
		c.logger.Warn("Failed to start image watcher, skipping watch",
			"paths", c.config.Paths,
			"error", err)
		cancel()
		return nil
	...
	c.logger.Info("Image-source fsnotify watching started", "paths", c.config.Paths)
...
// handleChangeEvent processes a change event from the image handler's watch channel.
// It prefers event.EntityStates (populated by enrichEvent when org is set) and
// falls back to an empty publish when only delete events arrive.
func (c *Component) handleChangeEvent(ctx context.Context, event handler.ChangeEvent) {
	c.logger.Debug("Image-source change event received",
		"path", event.Path,
...
func (c *Component) Stop(_ time.Duration) error {
	...
	c.logger.Info("Image-source stopped",
		"entities_published", c.entitiesPublished.Load(),
...
// InputPorts returns an empty slice — image-source generates data from the filesystem.
