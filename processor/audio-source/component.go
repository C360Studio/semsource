...
// audioSourceSchema defines the configuration schema for the audio-source component.
var audioSourceSchema = component.GenerateConfigSchema(reflect.TypeOf(Config{}))

// sourceCfg is a minimal handler.SourceConfig adapter for the audio handler.
...
func (s *sourceCfg) GetType() string             { return "audio" }
...
// Component implements the audio-source processor.
// It delegates all filesystem operations to the existing handler/audio package,
// which handles directory walking, ffprobe metadata extraction, and fsnotify
...
func NewComponent(rawConfig json.RawMessage, deps component.Dependencies) (component.Discoverable, error) {
	...
	if config.FileStoreRoot != "" {
		...
		fs = s
		opts = append(opts, audiohandler.WithStore(s))
	}

	h := audiohandler.New(opts...)
...
// Start performs the initial audio ingest across all configured paths, then
...
func (c *Component) Start(ctx context.Context) error {
	...
	c.logger.Info("Starting audio-source initial ingest",
		"paths", c.config.Paths,
		"org", c.config.Org,
		"watch_enabled", c.config.WatchEnabled)

	if err := c.ingestOnce(ctx); err != nil {
		return fmt.Errorf("initial audio ingest failed: %w", err)
	}

	c.logger.Info("Audio-source initial ingest complete",
		"paths", c.config.Paths,
...
// ingestOnce runs a single ingest pass: calls IngestEntityStates on the audio
...
func (c *Component) ingestOnce(ctx context.Context) error {
	...
	for _, state := range states {
		...
		if err != nil {
			c.logger.Warn("Invalid audio entity state",
				"id", state.ID,
				"error", err)
			c.ingestErrors.Add(1)
			continue
		...
		if err := c.publishEntity(ctx, payload); err != nil {
			c.logger.Warn("Failed to publish audio entity",
				"id", state.ID,
				"error", err)
			c.ingestErrors.Add(1)
			continue
...
// paths and re-publishes changed audio entities. Returns the cancel func for
...
func (c *Component) startWatching(ctx context.Context) context.CancelFunc {
	...
	if err != nil {
		c.logger.Warn("Failed to start audio watcher, skipping watch",
			"paths", c.config.Paths,
			"error", err)
		cancel()
		return nil
...
// handleChangeEvent processes a change event from the audio handler's watch channel.
...
func (c *Component) handleChangeEvent(ctx context.Context, event handler.ChangeEvent) {
	c.logger.Debug("Audio-source change event received",
		"path", event.Path,
	...
	for _, state := range event.EntityStates {
		payload, err := entityStateToPayload(state)
		if err != nil {
			c.logger.Warn("Invalid audio entity state on change",
				"id", state.ID,
				"error", err)
			c.ingestErrors.Add(1)
			continue
		...
		if err := c.publishEntity(ctx, payload); err != nil {
			c.logger.Warn("Failed to publish audio entity on change",
				"id", state.ID,
				"error", err)
			c.ingestErrors.Add(1)
			continue
...
// InputPorts returns an empty slice — audio-source generates data from the filesystem.
