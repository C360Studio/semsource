...
func (h *AudioHandler) Watch(ctx context.Context, cfg handler.SourceConfig) (<-chan handler.ChangeEvent, error) {
	...
	if len(roots) == 0 {
		return nil, fmt.Errorf("audio handler: Watch requires at least one configured path")
	}
	...
	for _, root := range roots {
		...
		if err != nil {
			...
			return nil, fmt.Errorf("audio handler: create watcher for %q: %w", root, err)
		}
		if err := w.Start(ctx); err != nil {
			...
			_ = w.Stop()
			return nil, fmt.Errorf("audio handler: start watcher for %q: %w", root, err)
		}
