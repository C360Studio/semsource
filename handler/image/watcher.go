...
func (h *ImageHandler) Watch(ctx context.Context, cfg handler.SourceConfig) (<-chan handler.ChangeEvent, error) {
	...
	root := cfg.GetPath()
	if root == "" {
		return nil, fmt.Errorf("image handler: Watch requires a non-empty path")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("image handler: create watcher: %w", err)
	}

	if err := w.Start(ctx); err != nil {
		return nil, fmt.Errorf("image handler: start watcher: %w", err)
	}
