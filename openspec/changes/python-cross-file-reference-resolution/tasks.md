...
- [x] 1.1 Add an import parser in `source/ast/python/` that walks a file's
...
- [x] 1.2 Unit-test the import parser against each binding form (including aliases
  and multi-level relative imports); assert the produced bindings.

## 2. Module → file resolver

- [x] 2.1 Add a `moduleToRelPath(module string, fromFile string, level int) (relPath string, ok bool)`
...
- [x] 2.2 Unit-test the resolver: absolute in-tree module → `.py` and `__init__.py`
...
- [x] 3.1 Thread the file's import bindings into `typeNameToEntityID` (or a small
...
- [x] 3.2 Ensure the defining-file `relPath` is normalized exactly as `ParseFile`
  normalizes a parsed file's path (relative to `repoRoot`), so ref-id == def-id.

## 4. Tests

- [x] 4.1 Parser regression: parse two files together (`pkg/base.py` defines
...
- [x] 4.2 Negative tests: third-party import stays `external:`/inert; a star-import
  reference is not resolved to a wrong entity; an unresolvable relative import is inert.
- [x] 4.3 Confirm same-file resolution (task #43) is unchanged (existing
  `TestParseFile_ClassInheritance` still passes).

## 5. Live validation

- [x] 5.1 Index a real multi-module Python repo (httpx) via the compose stack;
...
- [x] 6.1 `go build ./...`, `go vet`, `revive` (zero warnings), full `go test ./...` green.
- [x] 6.2 Update in-code comments to reflect cross-file now resolves; note remaining
