...
- [ ] 3.1 Generalise the guard in `processor/code-context/component.go` from
...
- [ ] 3.2 Preserve the existing protection that the guard never empties a result set
- [ ] 3.3 Add a Go test covering a body-less non-document entity (the
...
- [ ] 5.1 Bump `questions.json` to version 3
- [ ] 5.2 Add a README comparability note naming the two reasons v2 results do not carry across
...
- [ ] 6.1 Reduce the order-dependent drop to a minimal reproduction script that does not depend
      on the scorecard (burst of diverse `doc_context` queries, then the target query once)
- [ ] 6.2 Add the entry to `docs/upstream/semstreams-asks.md`, triaged framework-shaped, naming
...
- [ ] 6.3 Include the second and more valuable half of the ask: fusion exposes no score, rank or
...
- [ ] 6.5 State the blast radius accurately: reproduced on `doc_context`; `code_search` was
      stable across 4 consecutive calls after the same burst, which is not proof of immunity

## 7. Gates

- [ ] 7.1 `task lint` clean — revive warnings fail CI, pinned v1.15.0
- [ ] 7.2 `go test ./...` and `go test -race ./...` for the group-3 product change
- [ ] 7.3 `shellcheck` or equivalent review of the `run.sh` changes, since the defect this change
