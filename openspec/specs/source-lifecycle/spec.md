...
## Purpose
Source registration is a verifiable contract: add returns real handles with a
machine-checkable readiness condition, removal actually deregisters (manifest,
status aggregation, component teardown via the framework KV watch) and is
observable within one aggregation pass, unknown handles fail with NOT_FOUND,
and the manifest always mirrors the running component set (audit 2026-07-19).
