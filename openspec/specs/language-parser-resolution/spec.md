# language-parser-resolution Specification

## Purpose
Which parser reads a given file decides that file's `{domain}` segment, and domain is part of every
entity ID it produces — so parser selection is an identity concern, not a dispatch detail. This
capability covers how a file is routed to a language, and the two guarantees that keeps: the answer
is a deterministic function of declared configuration rather than of file contents or map order, and
a language that cannot be mapped fails loudly instead of silently becoming another one.

Routing is resolved once per watch path from the declared language set
(`processor/ast-source/routing.go`), and the language tables it depends on are data rather than
switch statements (`handler/ast/languages.go`) so that a partly-wired language cannot pass unnoticed.
