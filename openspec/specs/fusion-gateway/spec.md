...
(ADR-0004). This spec is seeded lazily — it records the gateway behavior that
changes have touched to date (domain-scoped NL retrieval, the HTTP error
envelope and its failure classification, impact seed resolution, and
passage-scoped doc evidence) and is not a full description of the gateway.
## Requirements
