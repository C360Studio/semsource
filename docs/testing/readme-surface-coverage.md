...
| Explicit `docker-compose.tier2-dev.yml` overlay                           | `task tier2:smoke:dev` asserts the exact 4-service set, image pins, and tier-2 config wiring    | partial  | Bring-up + inference gated behind `SEMSOURCE_TIER2_SMOKE_FULL=1`; not run in CI |
...
| `graph_summary`                       | `TestIntegration_GraphSummaryReturnsTheSubstrateShape` (asserts the substrate's entity-type/predicate shape, so a reintroduced competing handler fails); `task core:smoke` asserts the same on a live stack | covered  | None      |
| Request-subject ownership             | `TestNoSemSourceSubjectCollidesWithTheSubstrate` (verified failing before the fix, naming `graph.query.summary`) | covered  | Compares against SemSource's declaration of the substrate surface, not semstreams' unexported registrations — upstream ask filed |
| `graph_search`                        | `TestIntegration_GraphSearchRanksAndDiscloses` (asserts a ranked list survives a digest-less substrate response, and that triples do not leak); `TestDeriveDisclosureAcrossTheCapabilityLadder` covers all three rungs from recorded substrate responses; `task core:smoke` asserts the answer and its not-community-backed disclosure | covered  | Community/LLM rungs are proven from recorded responses, not a live clustered stack — the deferred `community_context` change brings the live tier-2 harness |
